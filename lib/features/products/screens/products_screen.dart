...
class _ProductsScreenState extends State<ProductsScreen> with SingleTickerProviderStateMixin {
 late List<Color> colors;
  @override
  void initState() {
    colors = List.generate(widget.allCategoriesModel?.count??0, (index) => AppColors.black);
    context.read<ProductsCubit>().tabController = TabController(length: widget.allCategoriesModel!.count!+1, vsync: this);
    super.initState();
                          ...
                          if(index==0){
                            ...
                            return  InkWell(
                              onTap: () async {
                                ...
                                cubit.tabController!.index=index;
                                await  context.read<ProductsCubit>().getProductsByCategoryId(widget.allCategoriesModel?.result?[index-1].id??0);

                              },
                              child: Text(
                                widget.allCategoriesModel!.result![index-1].name!,
            ...
            // ),
              ...
              height: 74.h,
               ...
               state is LoadingAllProductState?
                  ...
                  itemCount:context.read<HomeCubit>().allProductsModel?.count??0 ,
                  itemBuilder: (context, index) {
                    return HomeProductItem2(productModel:ProductModel(
                        name:context.read<HomeCubit>().allProductsModel?.result?[index].name,
                        image: context.read<HomeCubit>().allProductsModel?.result?[index].image1920,
                        price: context.read<HomeCubit>().allProductsModel?.result?[index].listPrice,
                        // unit: cubit.productsByCategoryIdModel!.result?[index].currencyId.toString(),
                        unit: "kg",
                        id: context.read<HomeCubit>().allProductsModel?.result?[index].id) ,
                    );
                  },):
...
//   void initState() {
//     colors = List.generate(widget.allCategoriesModel?.count??0, (index) => AppColors.black);
//     controller = TabController(length: widget.allCategoriesModel?.count??0, vsync: this);
//     super.initState();
//
//     context.read<ProductsCubit>().getProductsByCategoryId(0);
//   }
...
//                             tabs:
//                             List.generate(widget.allCategoriesModel?.count??0, (index) =>   Text(widget.allCategoriesModel!.result![controller?.index??0].name!,
//                           style: TextStyle(
//                               color:colors[controller?.index??0],
//                               fontWeight: FontWeight.w600,
//                               fontSize: getSize(context)*0.04 ),),)
//                 )
//                         // Expanded(
